...
- The spec is `.scratch/presence/spec.md` (written 2026-09-14). Its seven Spec cards, op-043 to op-049, are approved. The 14 tickets are in `.scratch/presence/issues/`, blockers first. 01 done; 02, 03, 04 and 14 wait on a human (Susan's company-page copy, Lee's yes on the LinkedIn copy, Susan's first Pages CMS sign-in, Lee's Formspree check); 12 and 13 are `wontfix` after op-065; 05 to 11 are ready for an agent, in order.
...
- Next: `/implement-lee presence` does not exist yet; use `/implement ticket 05`, then 06 to 11 in order, `/clear` between tickets. Pushes to `main` need the RPPLee account (`gh auth switch -u RPPLee && gh auth setup-git`, then switch back to lbm54).
