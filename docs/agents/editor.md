...
| Services | `_services/*.md` | Title, tagline, homepage card text, price, duration, format, Stripe link, banner, the Request a time button and its words, group, order and the page text of each service. Order is a number; lower comes first within the group. The group (Individuals, Groups, Organizations, or Not on the homepage) is where the service sits on the homepage. Prices are free text, shown as written. |
| BizBlitz and VizBlitz page | `pages/blitz.html` | The /blitz/ page: the words at the top, a list of sessions (name, tagline, description, facts, links) and the closing section. The HTML is `_layouts/programs.html`. |
...
| Services page | `pages/services.html` | The /services/ page: the heading, the line under it and the closing section. The cards are the services themselves. `/programs/` forwards here since 2026-09-24. |
