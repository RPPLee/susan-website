...
//
// Spec shapes (JSON):
//   { kind: 'flow', title?, rows: [[{id, label, tone?}], ...], edges: [{from, to, label?, tone?}], example?: [lines] }
//   { kind: 'timeline', title?, steps: [{at, label, tone?}], example?: [lines] }
// `label` may hold `\n` for a second line. `tone` is one of ink (default),
// accent, ok, no, pending, amend, muted. The page shows a picture about
...
export const TOKENS = ['ground', 'ground-2', 'line', 'ink', 'ink-2', 'muted', 'accent', 'accent-ink', 'ok', 'ok-soft', 'no', 'no-soft', 'pending', 'pending-soft', 'amend', 'amend-soft', 'card', 'focus'];

// Light-theme fallbacks, so the file reads on its own in an editor.
...
function flow(spec) {
  ...
  rows.forEach((row, r) => {
    ...
    const rowW = sized.reduce((a, n) => a + n.w, 0) + gap * (sized.length - 1);
    boxes.push({ sized, rowW, r });
  });
  ...
  const bottom = top + (rows.length - 1) * rowH + Math.max(...[...nodes.values()].map(n => n.h)) + 16;
