...
Nothing enters a file here except on a ratifier's verdict (Lee or Susan), given on the decisions page.
Agents never edit these files by hand; the sync module in `_page/` applies verdicts.

This repo is the Jekyll site for Metaphase Management Associates. Features here are tracks of
Susan's online presence (`presence` covers the website, LinkedIn, Substack and payments); the
tooling is the same as in `mealvana_endurance`, and fixes should be made in both.
...
Artifact: https://claude.ai/code/artifact/2a9c0397-9f1a-4380-943d-e55bf6ab2e31
...
- svg: <repo path of a drawn diagram, only on a screenless card; see Drawn pictures>
...
name: `> 2026-09-14 approved by Lee`, `> 2026-09-14 rejected by Susan: too early`. A verdict
...
blockers and next line come from the three header lines every ticket carries, and every `op-NNN`
...
its ids, and a closing `Next:` line. `/implement-lee` does not exist yet.
